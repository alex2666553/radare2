...
static void free_mlist(struct mlist *mlist) {
	...
	for (ml = mlist->next; ml != mlist;) {
		...
		r_magic_file_delmagic (mg, ml->mapped, ml->nmagic);
		free (ml);
		ml = next;
	}
	free (ml);
...
static int info_from_stat(RMagic *ms, unsigned short md) {
	...
	if (md & 0222) {
		if (r_magic_file_printf (ms, "writable, ") == -1) {
			return -1;
		}
	}
	if (md & 0111) {
		if (r_magic_file_printf (ms, "executable, ") == -1) {
			return -1;
		}
	}
	if (S_ISREG (md)) {
		if (r_magic_file_printf (ms, "regular file, ") == -1) {
			return -1;
		}
	}
	if (r_magic_file_printf (ms, "no read permission") == -1) {
		return -1;
	}
	return 0;
}
...
#define SLOP (1 + sizeof (union VALUETYPE))
	...
	if (r_magic_file_reset (ms) == -1) {
		goto done;
	}

	switch (r_magic_file_fsmagic (ms, inname, &sb)) {
	...
	if (ispipe) {
		...
		if ((nbytes = read(fd, (char *)buf, HOWMANY)) == -1) {
			r_magic_file_error(ms, errno, "cannot read `%s'", inname);
			goto done;
		}
#ifdef O_NONBLOCK
	}
	...
	if (r_magic_file_buffer (ms, fd, inname, buf, (size_t)nbytes) == -1) {
		goto done;
	}
	rv = 0;
done:
	...
	return rv == 0 ? r_magic_file_getbuffer(ms) : NULL;
}
...
R_API bool r_magic_load_buffer(RMagic* ms, const ut8 *magicdata, size_t magicdata_size) {
	if (magicdata_size > 0 && *magicdata == '#') {
		struct mlist *ml = r_magic_file_apprentice (ms, (const char *)magicdata, magicdata_size, FILE_LOAD);
		if (ml) {
			free_mlist (ms->mlist);
			ms->mlist = ml;
			return true;
...
R_API bool r_magic_load(RMagic* ms, const char *magicfile) {
	struct mlist *ml = r_magic_file_apprentice (ms, magicfile, strlen (magicfile), FILE_LOAD);
	if (ml) {
		free_mlist (ms->mlist);
		ms->mlist = ml;
		return true;
...
R_API bool r_magic_compile(RMagic *ms, const char *magicfile) {
	struct mlist *ml = r_magic_file_apprentice (ms, magicfile, strlen (magicfile), FILE_COMPILE);
	free_mlist (ml);
	return ml;
}

R_API bool r_magic_check(RMagic *ms, const char *magicfile) {
	struct mlist *ml = r_magic_file_apprentice (ms, magicfile, strlen (magicfile), FILE_CHECK);
	free_mlist (ml);
	return ml;
}
...
R_API const char *r_magic_buffer(RMagic *ms, const void *buf, size_t nb) {
	...
	if (r_magic_file_buffer (ms, -1, NULL, buf, nb) == -1) {
		return NULL;
	}
	return r_magic_file_getbuffer (ms);
}
